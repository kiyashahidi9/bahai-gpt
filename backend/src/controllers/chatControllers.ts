...
import { type AuthenticationRequest } from '../utils/auth.js'
import {
    queryAllConversations,
    queryAllChats,
    queryNewConversation,
    queryNewChat,
    queryDeleteConversation,
    queryUpdatedConversationTitle,
...
export async function getAllConversations(req: Request, res: Response) {
    ...
    if (!userId) throw new AuthorizationError("Authorization required")

    const conversations = await queryAllConversations(userId)
    res.status(200).json(conversations)
}

export async function getAllChatsForConversation(req: Request, res: Response) {
    const authReq = req as AuthenticationRequest
    const userId = authReq.user?.id
    if (!userId) throw new AuthorizationError("Authorization required")
...
export async function createNewChatForConversation(req: Request, res: Response) {
    const authReq = req as AuthenticationRequest
    const userId = authReq.user?.id
    if (!userId) throw new AuthorizationError("Authorization required")
